...
###sudo apt install build-essential gdb -y #开发
...
sudo apt install curl aria2 -y #下载
...
###sudo apt install kdenlive breeze -y #视频剪辑
###sudo apt install krita krita-l10n -y #数字绘画
sudo apt install featherpad -y #文本编辑器
...
###sudo apt install tigervnc-scraping-server tigervnc-standalone-server tigervnc-viewer -y #远程桌面
